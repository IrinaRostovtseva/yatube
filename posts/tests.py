...
class TestNewPost(TestCase):
    ...
    def test_not_auth_user(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(reverse('login'), redirect_url)
...
class TestEditPost(TestCase):
    ...
    def test_post_editing(self):
        ...
        response_post = self.client.post(
            reverse('post_edit',
                    args=[self.user.username, self.post.id]),
            {'author': self.post.author, 'text': 'I edit this text'},
            follow=True)
...
class TestDeletePost(TestCase):
    ...
    def test_not_auth_user(self):
        response = self.client.get(
            reverse('post_delete', args=[self.user.username, self.post.id]),
            follow=True)
...
class TestImageUpload(TestCase):
    ...
    def test_image_being(self):
        with open(os.path.join(settings.BASE_DIR, 'test_pic.jpg'), 'rb') as fp:
            self.client.post(
                reverse('new_post'),
                {'text': 'post with pic', 'group': self.group.id,
                 'image': fp},
                follow=True)
    ...
    def test_not_image_upload(self):
        with open(os.path.join(settings.BASE_DIR,
                               'requirements.txt'), 'rb') as fp:
            self.client.post(
                reverse('new_post'),
                {'text': 'post with non pic', 'image': fp})
...
class TestIndexPageCache(TestCase):
    ...
    def test_index_cache(self):
        Post.objects.create(author=self.user, text='Test post 1')
        self.client.get(reverse('index'))
        self.assertIn('Test post 1', self.client.get(
            reverse('index')).content.decode())
        Post.objects.create(author=self.user, text='Test post 2')
    ...
    def test_not_auth_comment(self):
        response = self.client.post(
            reverse('add_comment',
                    args=[self.user.username, self.post.id]),
            {'post': self.post,
             'text': 'I am not logged in, but I wanna comment'},
            follow=True)
        redirect_url = f'/{response.redirect_chain[0][0].split("?")[0]}/'
        self.assertEqual(reverse('login'), redirect_url)
    ...
    def test_auth_comment(self):
        ...
        self.client.post(
            ...
            {'post': self.post, 'author': self.user,
             'text': 'Hey, I am logged in. Cool post'},
            follow=True)
        response = self.client.get(
